...
func (c *Config) expandEnvVars() {
	...
	// APK specific (metadata in .PKGINFO)
	c.APK.Pkgname = os.Expand(c.APK.Pkgname, c.envMappingFunc)
	c.APK.Pkgver = os.Expand(c.APK.Pkgver, c.envMappingFunc)
	c.APK.Packager = os.Expand(c.APK.Packager, c.envMappingFunc)
	c.APK.Origin = os.Expand(c.APK.Origin, c.envMappingFunc)
	c.APK.Commit = os.Expand(c.APK.Commit, c.envMappingFunc)
	c.APK.Triggers = c.expandEnvVarsStringSlice(c.APK.Triggers)
	c.APK.InstallIf = c.expandEnvVarsStringSlice(c.APK.InstallIf)
	c.APK.PkginfoGenerator = os.Expand(c.APK.PkginfoGenerator, c.envMappingFunc)
	c.APK.PkginfoGeneratorVersion = os.Expand(c.APK.PkginfoGeneratorVersion, c.envMappingFunc)
...
type APK struct {
	...
	// Pkgname, if set, overrides the top-level name in .PKGINFO only.
	Pkgname string `yaml:"pkgname,omitempty" json:"pkgname,omitempty" jsonschema:"title=package name in .PKGINFO, defaults to name"`
	// Pkgver, if set, overrides the computed apk version string in .PKGINFO only.
	Pkgver string `yaml:"pkgver,omitempty" json:"pkgver,omitempty" jsonschema:"title=package version in .PKGINFO"`
	// BuildDate is the unix timestamp written as builddate in .PKGINFO.
	BuildDate *int64 `yaml:"builddate,omitempty" json:"builddate,omitempty" jsonschema:"title=package build time as unix seconds"`
	// Packager is the person or entity that built the package (builddate/packager in apkbuild).
	Packager string `yaml:"packager,omitempty" json:"packager,omitempty" jsonschema:"title=packager field in .PKGINFO"`
	// Origin is the origin package name (split packages, etc.).
	Origin string `yaml:"origin,omitempty" json:"origin,omitempty" jsonschema:"title=origin in .PKGINFO"`
	// Commit is a source revision identifier (e.g. git commit hash).
	Commit string `yaml:"commit,omitempty" json:"commit,omitempty" jsonschema:"title=commit in .PKGINFO"`
	// ReplacesPriority is the replaces priority in .PKGINFO.
	ReplacesPriority *int `yaml:"replaces_priority,omitempty" json:"replaces_priority,omitempty" jsonschema:"title=replaces priority in .PKGINFO"`
	// ProviderPriority is the provider priority in .PKGINFO.
	ProviderPriority *int `yaml:"provider_priority,omitempty" json:"provider_priority,omitempty" jsonschema:"title=provider priority in .PKGINFO"`
	// Triggers lists trigger dependencies for .PKGINFO.
	Triggers []string `yaml:"triggers,omitempty" json:"triggers,omitempty" jsonschema:"title=triggers lines in .PKGINFO"`
	// InstallIf lists install_if conditions for .PKGINFO.
	InstallIf []string `yaml:"install_if,omitempty" json:"install_if,omitempty" jsonschema:"title=install_if lines in .PKGINFO"`
	// PkginfoGenerator, if set, adds a two-line comment header to .PKGINFO: "# Generated by <name> [<version>]" and "# <mtime in Unix date form, UTC>" using top-level mtime.
	PkginfoGenerator        string `yaml:"pkginfo_generator,omitempty" json:"pkginfo_generator,omitempty" jsonschema:"title=PKGINFO generator label for comment header"`
	PkginfoGeneratorVersion string `yaml:"pkginfo_generator_version,omitempty" json:"pkginfo_generator_version,omitempty" jsonschema:"title=optional version for PKGINFO generator line"`
}
