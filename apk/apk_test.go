...
func TestPKGINFOExtendedFields(t *testing.T) {
	ts := int64(1712659200)
	rp := 10
	pp := 20
	info := exampleInfo()
	info.License = "MIT"
	info.APK.Pkgname = "override-pkg"
	info.APK.Pkgver = "9.9.9-r99"
	info.APK.BuildDate = &ts
	info.APK.Packager = "Alice Builder <alice@example.com>"
	info.APK.Origin = "foo-origin"
	info.APK.Commit = "abc123def456"
	info.APK.ReplacesPriority = &rp
	info.APK.ProviderPriority = &pp
	info.APK.Triggers = []string{"/bin/sh", "/etc/apk/foo"}
	info.APK.InstallIf = []string{"openrc", "!udev"}

	var w bytes.Buffer
	require.NoError(t, writeControl(&w, controlData{
		Info:          info,
		InstalledSize: 424242,
		Datahash:      strings.Repeat("ab", 32), // 64 hex chars like sha256
	}))
	golden := "testdata/TestPKGINFOExtendedFields.golden"
	if *update {
		require.NoError(t, os.WriteFile(golden, w.Bytes(), 0o655)) // nolint: gosec
	}
	bts, err := os.ReadFile(golden) //nolint:gosec
...
func TestPKGINFOPriorityZero(t *testing.T) {
	z := 0
	info := exampleInfo()
	info.APK.ReplacesPriority = &z
	info.APK.ProviderPriority = &z
	...
	require.Contains(t, w.String(), "replaces_priority = 0\n")
	require.Contains(t, w.String(), "provider_priority = 0\n")
}

func TestPKGINFOGeneratorHeader(t *testing.T) {
	info := exampleInfo()
	info.MTime = time.Date(2022, 7, 6, 19, 9, 49, 0, time.UTC)
	info.APK.PkginfoGenerator = "dhi/build-pkg"
	info.APK.PkginfoGeneratorVersion = "1.2.3"
	...
	wantPrefix := "# Generated by dhi/build-pkg 1.2.3\n# Wed Jul  6 19:09:49 UTC 2022\npkgname = "
	require.True(t, strings.HasPrefix(w.String(), wantPrefix), w.String())
}

func TestPKGINFOGeneratorHeaderNoVersion(t *testing.T) {
	info := exampleInfo()
	info.MTime = time.Date(2022, 7, 6, 19, 9, 49, 0, time.UTC)
	info.APK.PkginfoGenerator = "dhi/build-pkg"
	...
	wantPrefix := "# Generated by dhi/build-pkg\n# Wed Jul  6 19:09:49 UTC 2022\npkgname = "
	require.True(t, strings.HasPrefix(w.String(), wantPrefix), w.String())
}
...
func TestAPKConventionalFileName(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			Expect: "default_1.2.3_p1-r10_x86.apk",
		},
		{
		...
		},
		{
			...
			Expect: "default_1.2.3a_alpha1_git~aaaccc-r47_x86_64.apk",
		},
	}
...
// See apk-tools src/version.c (apk version string) and Alpine APKBUILD Reference (pkgver).
func TestPkgverAlpineSpec(t *testing.T) {
	t.Parallel()
	for _, tc := range []struct {
		...
		{
			name: "stable",
			info: &nfpm.Info{Version: "1.35.0", Release: "18"},
			want: "1.35.0-r18",
		},
		{
			name: "prerelease_beta",
			info: &nfpm.Info{Version: "1.0.0", Prerelease: "beta1", Release: "0"},
			want: "1.0.0_beta1-r0",
		},
		{
			name: "git_snapshot_date",
			info: &nfpm.Info{Version: "1.0.0", VersionMetadata: "git20180204", Release: "0"},
			want: "1.0.0_git20180204-r0",
		},
		{
			name: "metadata_hex_only",
			info: &nfpm.Info{Version: "2.0.0", VersionMetadata: "abcdef0123456", Release: "1"},
			want: "2.0.0~abcdef0123456-r1",
		},
		{
			name: "patch_meta",
			info: &nfpm.Info{Version: "1.2.3", VersionMetadata: "2", Release: "3"},
			want: "1.2.3_p2-r3",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			require.Equal(t, tc.want, pkgver(tc.info))
		})
	}
}
